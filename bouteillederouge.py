...
@app.route('/docs')
def docs():
    return render_template('base-iframe.html', iframe_src='http://{}:{}/{}/'.format(urlparse(request.url_root).hostname,
...
def monitoring():
    if not pm.running:
        flash(Markup(flash_msg['api_is_stop'][pm.config.info.langage].format('Monitor &amp; Control', url_for('logs'),
                                                                             url_for('APIstart'))), 'alert')
...
def monitor():
    if not pm.running:
        flash(Markup(flash_msg['api_is_stop'][pm.config.info.langage].format('Primitive Manager', url_for('logs'),
                                                                             url_for('APIstart'))), 'alert')
    return render_template(
        'base-iframe.html',
        iframe_src=url_for(
...
def move_recorder():
    if not pm.running:
        flash(Markup(flash_msg['api_is_stop'][pm.config.info.langage].format('Move Recorder', url_for('logs'),
                                                                             url_for('APIstart'))), 'alert')
        connect = False
        source = None
    else:
        ...
        try:
            ...
            connect = False
    return render_template(pm.config.info.langage + '/move-recorder.html', motors=pm._get_robot_motor_list(),
...
def viewer():
    if not pm.running:
        flash(Markup(flash_msg['api_is_stop'][pm.config.info.langage].format('Web Viewer', url_for('logs'),
                                                                             url_for('APIstart'))), 'alert')
    return render_template(
        'base-iframe.html',
        iframe_src='http://{}:{}/{}/#{}'.format(urlparse(request.url_root).hostname, pm.config.poppyPort.viewer,
                                                pm.config.robot.creature, pm.config.poppyPort.http)
    )
...
def camera():
    if not pm.running:
        flash(Markup(flash_msg['api_is_stop'][pm.config.info.langage].format('Web Camera', url_for('logs'),
                                                                             url_for('APIstart'))), 'alert')
    return render_template('camera.html', source='http://{}:{}/frame.png'.format(urlparse(request.url_root).hostname,
                                                                                 pm.config.poppyPort.snap), FPS=6)
...
def snap():
    if not pm.running:
        flash(Markup(
            flash_msg['api_is_stop'][pm.config.info.langage].format('Snap', url_for('logs'), url_for('APIstart'))),
            'alert')
    return render_template(
        'base-iframe.html',
...
def jupyter():
    default_notebook = 'http://{}:{}/notebooks/My%20Documents/Python%20notebooks/Discover%20your%20{}.ipynb'.format(
        urlparse(request.url_root).hostname, pm.config.poppyPort.jupyter,
        pm.config.robot.creature.replace('-', ' ').title().replace(' ', '%20'))
    if pm.running:
        flash(Markup(flash_msg['api_is_start'][pm.config.info.langage].format("Jupyter Notebook", url_for('logs'),
...
def AnotherLanguage():
    if pm.running:
        flash(Markup(
            flash_msg['api_is_start'][pm.config.info.langage].format("another programming language", url_for('logs'),
                                                                     url_for('APIstop'))), 'alert')
    return render_template('base-iframe.html',
                           iframe_src='http://{}:{}/notebooks/My%20Documents/Python%20notebooks/Another%20language.ipynb'.format(
                               urlparse(request.url_root).hostname, pm.config.poppyPort.jupyter))
...
def scratch():
    if not pm.running:
        flash(Markup(
            flash_msg['api_is_stop'][pm.config.info.langage].format('Scratch', url_for('logs'), url_for('APIstart'))),
            'alert')
    return render_template(
        'base-iframe.html',
...
def MyDoc():
    return render_template('base-iframe.html',
                           iframe_src='http://{}:{}/tree/My%20Documents'.format(urlparse(request.url_root).hostname,
                                                                                pm.config.poppyPort.jupyter))
    ...
    for key, value in request.form.items():
        if value != '':
            ...
            if value != getattr(getattr(pm.config, key[0]), key[1]):
                ...
                msg += '<br>'
    if msg == '':
        flash(flash_msg['no_changed'][pm.config.info.langage], 'warning')
    else:
...
def terminal():
    ...
    return render_template(
        'base-iframe.html',
        iframe_src='http://{}:{}/terminals/poppy'.format(urlparse(request.url_root).hostname,
                                                         pm.config.poppyPort.jupyter)
    )
